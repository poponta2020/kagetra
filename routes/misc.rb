...
class MainApp < Sinatra::Base
  ...
  before do
    ...
    return if ["/public/","/api/user/auth/","/api/board_message/","/api/line_webhook_test","/js/","/img/","/css/"].any?{|s|path.start_with?(s)} or ["/","/robots.txt","/select_other_uid","/mobile/"].include?(path)
    @user = get_user
    if @user.nil? then
